...
The sidebar, the page head and a rail beside the content share one ground, `--cms-color-surface`, the sidebar through `--cms-color-rail` and the node inspector through `--cms-inspector-bg`, and no line separates them. The content area sits on `--cms-pane-bg` as an inset in that frame, and its top corners curve into the frame with `--cms-pane-radius`. The frame also shades the content along its edges with `--cms-pane-shadow`, the frame's own shadow turned inward; a theme sets it to `none` for a flat inset. A page head draws the inset so it stays put while the content scrolls under the head: its `::before` fills the ground outside the two curves, and its `::after` lays a rounded, transparent sheet carrying the shadow over the content's visible part; a page with a rail on the right, such as the node editor with its inspector, sets `--cms-pane-right` to the rail's width so the right curve lands on the content's edge, and the node editor switches it to `--cms-inspector-collapsed-width` while its inspector is collapsed, with the curve travelling along. The media library draws its inset directly on the bounded grid pane, leaving both the filter rail and file inspector on the surface ground. Its search, count and upload controls belong to the library island but mount into the server-rendered page head through `[data-media-toolbar]`, and are removed with the island. The collection list is a surface-coloured screen without an inset, so it keeps a line under its head instead. Below the 52rem breakpoint the curves are hidden.
