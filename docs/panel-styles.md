...
Every text input, select and textarea in the panel carries `.cms-input`, `.cms-select` or `.cms-textarea`, in PHP views and Svelte components alike, and those classes are the only place a control's border, fill, depth, focus and states are drawn. A component may size and place a control, never redraw it. There is no element-level control look: a bare `input` renders as the browser draws it, which makes a missing class obvious. Controls that are not form fields keep their own rules, such as the inverted code and source editors or the language select in the masthead.
