...
The shared [action-menu behavior](../panel/src/lib/action-menu.ts) works with both PHP and Svelte markup:
...
Ghost blocks, the buttons a multi-column canvas shows in its empty cells, are tab stops after the field's rows and before its add bar. Enter or Space inserts a block into that gap; with several block types it opens the picker at the ghost, which then behaves as the add bar's picker does.
