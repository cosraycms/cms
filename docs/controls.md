...
On a multi-column field the canvas shows its empty cells as ghost blocks: dashed buttons that each appear while the pointer is over their own cells or when they take focus. Clicking a ghost inserts a block that fills that gap, sized to it and placed before the row that follows the gap in reading order, so no other block moves; a gap left free by a row's indent is filled by taking that indent away. A field with one block type stamps it at once; otherwise the field's picker opens inside the gap, or beside it when the gap is too small, and the type chosen there or from the catalog lands in the gap. Gaps narrower than the field's minimum width, and gaps the browser's flow would pass over for an earlier free spot, get no ghost. Ghosts are measured from the rendered grid and are not part of the form.
