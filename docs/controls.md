...
Rows are **never collapsed** and carry **no header**. A block's chrome is a group hanging over its top right corner: a drag grip, the type label, a gear opening the block's settings dialog, and a menu with move up/down and remove. It appears on hover and on keyboard focus and is unclickable while hidden, so it never intercepts a click meant for the block beneath it.
