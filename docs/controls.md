...
Rows are **never collapsed** and carry **no header**. A block's chrome is a group hanging over its top right corner: a drag grip, the type label, a gear opening the block's settings dialog, and a menu with move up, move down, duplicate and remove. It appears on hover and on keyboard focus and is unclickable while hidden, so it never intercepts a click meant for the block beneath it.

What stays visible at rest is only what says something about the block that its content does not: today that is the row's locale pills, which sit in the same corner and do not move when the group appears. A block in an untranslated or asymmetric field therefore shows nothing at all until it is touched. New blocks come from a `+` that appears with the chrome: in a one-column list it sits on the seam above the hovered block; in a grid it sits on the block's start and end edges and inserts before or after it in order — a seam between grid cells cannot say where in the order a block would land once rows wrap, the edges of a block can. A permanently visible `+` at the foot appends. With several allowed types each `+` opens the type picker, with one it inserts at once. Adding, removing, reordering and renumbering is the shared repeater behavior, stamping from one inert `<template>` per allowed type; a stamped row focuses its first input. **Duplicate** stamps the block's own template right after it and copies the live values control by control — layout included, a media reference rather than the file, and for an element control such as rich text its payload as edited so far, read off the host rather than the last server-rendered value — under a fresh uid.
