...
Rows are **never collapsed** and carry **no header**. A block's chrome is a group hanging over its top right corner: a drag grip, the type label, a width stepper (only when the field has more than one column), a gear opening the row's `class`/`id` dialog, and a menu with insert above/below, move up/down, row and indent steppers and remove. It appears on hover and on keyboard focus and is unclickable while hidden, so it never intercepts a click meant for the block beneath it; a narrow block folds the width stepper into the menu (container query, no JS).
