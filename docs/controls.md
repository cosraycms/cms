...
Rows are **never collapsed** and carry **no header**. A block's chrome is a group hanging over its top right corner: a drag grip, the type label, a gear opening the block's settings dialog, and a menu with insert above/below, move up/down and remove. It appears on hover and on keyboard focus and is unclickable while hidden, so it never intercepts a click meant for the block beneath it.

What stays visible at rest is only what says something about the block that its content does not: today that is the row's locale pills, which sit in the same corner and do not move when the group appears. A block in an untranslated or asymmetric field therefore shows nothing at all until it is touched. New blocks come from a picker at the foot; with a single allowed type the picker is a plain add button. Adding, removing, reordering and renumbering is the shared repeater behavior, stamping from one inert `<template>` per allowed type; a stamped row focuses its first input.
