...
- Renamed the component token `--cms-node-max-width` to `--cms-editor-width` and narrowed it from `85rem` to the redesign's `55rem` editing column. A `panel.theme` stylesheet overriding the old name must rename its declaration; one that relied on the old default and wants the wide column back sets `--cms-editor-width: 85rem`. The unused tokens `--cms-node-topbar-height` and `--cms-editor-bg` were dropped.
